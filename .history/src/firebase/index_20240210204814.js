...
async function getCouponByValue(value) {
  ...
  if (res) {
    if (!res.used) {
      return { coupon: res.data(), message: "Kupon został aktywowany." };
    } else {
      return { coupon: res.data(), message: "Kupon został wcześniej użyty." };
    }
  } else {
    return { message: "Kod promocyjny nie istnieje." };
  }
}
