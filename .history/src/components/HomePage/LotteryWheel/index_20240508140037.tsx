...
export default function LotteryWheel({
  ...
  return (
    <>
      ...
      {lotteryWheelOpen && (
        <button
          className="fixed lg:right-12 top-12 z-[1500]"
          onClick={() => setLotteryWheelOpen(!lotteryWheelOpen)}
        >
      ...
      {lotteryWheelOpen && (
        <div className="h-[110vh] w-full fixed top-0 left-0 bg-black bg-opacity-85 z-[1000]">
          {prize.title !== "" && (
            <div className="fixed w-full h-full top-0 -left-1/2 translate-x-1/2">
              <Confetti width={1920} height={1019} />
            </div>
          )}
          <div className="overflow-x-hidden h-full md:py-12  w-full flex flex-col items-center justify-center">
            <div className="sm:bg-white flex-col flex rounded-3xl p-6 md:p-6 justify-center">
              <div className="flex flex-row w-full justify-between mb-3 ">
                <h2 className="text-2xl font-bold flex-row items-center text-zinc-800 hidden lg:flex">
                  Wylosuj promocję
                </h2>
                <button
              ...
              <button
                onClick={() => setPrizeListOpen(!prizeListOpen)}
                className="z-[500] hover:bg-indigo-500 fixed bottom-5 right-1 flex sm:hidden duration-200 bg-indigo-600 text-white rounded-3xl flex-row items-center justify-between font-bold"
              >
