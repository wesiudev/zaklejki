    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-3">
        {categories.map((item: any, i: any) => (
          <div
            key={i}
            className="p-3 drop-shadow-xl shadow-black rounded-lg aspect-square h-full w-full bg-indigo-200"
          >
            ...
            <div className="w-full flex items-end justify-center py-6">
              <Link
                href={`/sklep/${item.category}`}
                className="mt-6 bg-indigo-600 hover:bg-indigo-500 duration-300 rounded-md text-white px-4 py-2 w-full h-full"
              >
                Zobacz więcej
              </Link>
            </div>
